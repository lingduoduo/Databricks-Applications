...
with mlflow.start_run(experiment_id="625048133702327", run_name="lightgbm") as mlflow_run:
...
#         model.fit(X_train, y_train, classifier__callbacks=[lightgbm.early_stopping(5), lightgbm.log_evaluation(0)], classifier__eval_set=[(X_val_processed,y_val)], classifier__sample_weight=w_train)
        model.fit(X_train, y_train, classifier__eval_set=[(X_val_processed,y_val)], classifier__sample_weight=w_train)
