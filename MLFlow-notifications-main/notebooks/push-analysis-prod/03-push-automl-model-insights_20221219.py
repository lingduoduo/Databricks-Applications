...
# MAGIC This is an auto-generated notebook. To reproduce these results, attach this notebook to the **Ling Huang's Cluster** cluster and rerun it.
# MAGIC - Compare trials in the [MLflow experiment](#mlflow/experiments/625048133702327)
# MAGIC - Navigate to the parent notebook [here](#notebook/625048133702328) (If you launched the AutoML experiment using the Experiments UI, this link isn't very useful.)
...
silver_df = bronze_df_sample.withColumn(target_col, F.col(target_col).cast(BooleanType()))
...
supported_cols = ["cal_utc_day_of_week", 
                  "cal_utc_hour", 
                  "gift_count_24h", 
                  "search_match_count_24h", 
                  "view_end_count_24h", 
...
num_imputers = []
num_imputers.append(("impute_mean", SimpleImputer(), ["broadcast_search_count_24h", "gift_count_24h", "search_match_count_24h", "search_user_count_24h", "view_count_24h", "view_end_count_24h"]))
...
numerical_transformers = [("numerical", numerical_pipeline, ["gift_count_24h", "search_match_count_24h", "broadcast_search_count_24h", "search_user_count_24h", "view_count_24h", "view_end_count_24h"])]
...
categorical_one_hot_transformers = [("onehot", one_hot_pipeline, ["age", "cal_utc_day_of_week", "cal_utc_hour", "country", "device_type", "from_user_age", "from_user_country", "from_user_gender", "gender"])]
...
# MAGIC 
# MAGIC `_automl_split_col_81d4` contains the information of which set a given row belongs to.
...
# AutoML completed train - validation - test split internally and used _automl_split_col_81d4 to specify the set
...
# MAGIC - All the runs are logged under [this MLflow experiment](#mlflow/experiments/625048133702327)
...
def objective(params):
    with mlflow.start_run(experiment_id="625048133702327", run_name="xgboost") as mlflow_run:
...
space = {
  "colsample_bytree": 0.24746374603966168,
  "learning_rate": 0.8262264695729091,
  "max_depth": 9,
  "min_child_weight": 4,
  "n_estimators": 1649,
  "n_jobs": 100,
  "subsample": 0.32615160226410916,
  "verbosity": 0,
  "random_state": 542668491,
}
...
if shap_enabled:
    ...
    train_sample = X_train.sample(n=min(200, X_train.shape[0]), random_state=542668491)

    # Sample some rows from the validation set to explain. Increase the sample size for more thorough results.
    example = X_val.sample(n=min(100, X_val.shape[0]), random_state=542668491)
...
print(f"%md [Link to model run page](#mlflow/experiments/625048133702327/runs/{ mlflow_run.info.run_id }/artifactPath/model)")
