...
with mlflow.start_run(experiment_id="2695937635426936", run_name="lightgbm_fulldata") as mlflow_run:
...
#     model.fit(X=X_train, y=y_train, sample_weight=w_train, categorical_feature=categorical_features)
...
if shap_enabled:
    ...
    train_sample = X_train.sample(n=min(300, X_train.shape[0]), random_state=750994329)

    # Sample some rows from the validation set to explain. Increase the sample size for more thorough results.
    example = X_val.sample(n=min(300, X_val.shape[0]), random_state=750994329)
