...
fs.write_table("churndb.service_features", service_df, "merge") 
...
display(spark.read.table("churndb.service_features").summary())
...
display(service_df .summary())
dbutils.data.summarize(service_df)
...
display(service_df) # check out other Plot Options too
...
training_set = fs.create_training_set(spark.read.table("demographic"),
...
training_set.load_df().write  \
                      .format("delta") \
                      .mode("overwrite") \
                      .saveAsTable("churndb.demographic_service")
...
model_uri = 'runs:/941b4c772a2543da98e3985b17b85704/model'
