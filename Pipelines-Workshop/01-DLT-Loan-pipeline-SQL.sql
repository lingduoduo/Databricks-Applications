...
AS SELECT * FROM cloud_files('/demos/dlt/loans/ling/raw_transactions', 'json', map("cloudFiles.inferColumnTypes", "true"))
...
AS SELECT * FROM delta.`/demos/dlt/loans/ling/ref_accounting_treatment`
...
AS SELECT * FROM cloud_files('/demos/dlt/loans/ling/historical_loans', 'csv', map("cloudFiles.inferColumnTypes", "true"))
...
AS SELECT count(*) as sum_count, country_code FROM live.cleaned_new_txs GROUP BY country_code

-- COMMAND ----------

-- MAGIC %md ## Next steps
-- MAGIC
-- MAGIC Your DLT pipeline is ready to be started. <a dbdemos-pipeline-id="dlt-loans" href="/#joblist/pipelines/8a602548-36f7-49a8-bf0f-a856d85f783a">Click here to access the pipeline</a> created for you using this notebook.
-- MAGIC
