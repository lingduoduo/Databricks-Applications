...
Utilities	,
Water'''
cat = [category.strip('\n\t') for category in words.split(',')]

# COMMAND ----------

def categorize_ticker(desc: str) -> None:
    results = zero_shot_pipeline(
        desc,
        candidate_labels=cat,
    )
    display(pd.DataFrame(results)[:10])

# COMMAND ----------

categorize_ticker('''ticker: "AAL"	
merchants: ["American Airlines"]
category_list: ["Income","Air Travel","Investments","Paycheck"]
sic_codes: ["AIR TRANSPORTATION, SCHEDULED"]
top_category_list: ["Income","Investments","Travel"]
''')

# COMMAND ----------

categorize_ticker('''ticker: "AAN"	
merchants: ["["Aaron's"]"]
category_list: ["Furnishings"]
sic_codes:["SERVICES-EQUIPMENT RENTAL & LEASING, NEC"]
top_category_list: ["Home"]
''')
